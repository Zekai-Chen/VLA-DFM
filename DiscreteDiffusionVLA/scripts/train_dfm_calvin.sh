...
# DFM-VLA Supervised Fine-tuning script for CALVIN ABC->D
# Trains on the RLDS-converted CALVIN dataset (zhouhongyi/calvin_abc_rlds).
#
# Usage:
...
DATASET_NAME="${DATASET_NAME:-calvin_abc_rlds}"
RUN_ROOT="${RUN_ROOT:?RUN_ROOT required}"

# ── Embodiment-specific defaults (CALVIN: rgb_static + rgb_gripper) ────────
...
# ── Diagnostics ────────────────────────────────────────────────────
echo "========================================="
echo "DFM-VLA Supervised Fine-tuning (CALVIN)"
echo "  GPUs: $NPROC"
