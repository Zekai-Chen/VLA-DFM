...
# Optional DFM debug stats (training-time)
# export VLA_DFM_DEBUG=1
# export VLA_DFM_DEBUG_EVERY=200
...
RUN_ROOT_DIR="${BASE_DIR}/checkpoints/dfm-diffusion_like-20k-smoke"
...
DFM_T_MIN=0.0
DFM_T_MAX=1.0
# Unused for masked_ce, but kept for parity
...
if [[ "${USE_DFM}" == "true" ]]; then
  torchrun --standalone --nnodes 1 --nproc-per-node ${NPROC} vla-scripts/finetune.py \
    ...
    --run_id_note "dfm-diffusion_like-20k--smoke-2xA100--$(date +%Y%m%d_%H%M)" \
