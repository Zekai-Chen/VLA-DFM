...
#SBATCH --job-name=openvla-eval-smoke-dd-debug
#SBATCH --output=logs/openvla_eval_smoke_dd_debug_%j.out
#SBATCH --error=logs/openvla_eval_smoke_dd_debug_%j.err
...
LOG_DIR="${BASE_DIR}/logs/eval_smoke_debug/$(date +'%m%d_%H%M')"
mkdir -p "$LOG_DIR"
...
FORCE_GRIPPER_STEPS=${FORCE_GRIPPER_STEPS:-20}
