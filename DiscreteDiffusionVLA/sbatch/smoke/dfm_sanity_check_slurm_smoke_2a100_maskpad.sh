...
#SBATCH --mem=60G
#SBATCH --time=4:00:00
#SBATCH --job-name=dfm-sanity-smoke-maskpad
#SBATCH --output=logs/dfm_sanity_smoke_maskpad_%j.out
#SBATCH --error=logs/dfm_sanity_smoke_maskpad_%j.err
...
# --- Run params ---
CHECKPOINT_ROOT="/scratch/ywn1043/VLA-DFM/checkpoints/ddopenvla-libero-object-smoke-3k-maskfix-moremask-tmax0.9/openvla-7b+libero_object_no_noops+b4+lr-0.0005+lora-r16+dropout-0.0--smoke-2xA100-3k-tmax0.9--20260302_0217"
...
MASK_EMBED_OVERRIDE=${MASK_EMBED_OVERRIDE:-pad}

LOG_DIR="/scratch/ywn1043/VLA-DFM/logs/dfm_sanity/$(date +'%m%d_%H%M')"
mkdir -p "$LOG_DIR"

echo "Running DFM sanity check..."
python "${REPO_ROOT}/scripts/dfm_sanity_check.py" \
  ...
  --mask_embed_override "${MASK_EMBED_OVERRIDE}" \
  > "${LOG_DIR}/dfm_sanity.log" 2>&1

echo "Sanity check finished. Log: ${LOG_DIR}/dfm_sanity.log"
