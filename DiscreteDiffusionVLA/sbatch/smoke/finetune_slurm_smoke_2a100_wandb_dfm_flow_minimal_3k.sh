...
# Optional DFM training debug toggles:
...
if [[ "${USE_DFM}" == "true" ]]; then
  torchrun --standalone --nnodes 1 --nproc-per-node ${NPROC} vla-scripts/finetune.py \
    ...
    --run_id_note "dfm-flow-minimal-3k--smoke-2xA100--$(date +%Y%m%d_%H%M)" \
