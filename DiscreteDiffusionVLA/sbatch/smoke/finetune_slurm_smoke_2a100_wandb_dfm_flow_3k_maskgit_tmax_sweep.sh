...
#SBATCH --job-name=openvla-ft-smoke-3k-tmax-sweep
#SBATCH --output=logs/openvla_ft_smoke_3k_tmax_sweep_%A_%a.out
#SBATCH --error=logs/openvla_ft_smoke_3k_tmax_sweep_%A_%a.err
#SBATCH --array=0-3
...
TMAX_LIST=(0.6 0.7 0.8 0.9)
DFM_T_MAX=${TMAX_LIST[${SLURM_ARRAY_TASK_ID:-0}]}
DFM_T_MAX_TAG=$(echo "${DFM_T_MAX}" | sed 's/\\./p/')

export WANDB_NAME="openvla_ft_smoke_3k_tmax${DFM_T_MAX_TAG}_${SLURM_JOBID:-local}_${SLURM_ARRAY_TASK_ID:-0}"
...
RUN_ROOT_DIR="${BASE_DIR}/checkpoints/ddopenvla-libero-object-smoke-3k-maskfix-moremask-tmax${DFM_T_MAX_TAG}"
...
if [[ "${USE_DFM}" == "true" ]]; then
  torchrun --standalone --nnodes 1 --nproc-per-node ${NPROC} vla-scripts/finetune.py \
    ...
    --run_id_note "smoke-2xA100-3k-tmax${DFM_T_MAX_TAG}--$(date +%Y%m%d_%H%M)" \
  ...
  torchrun --standalone --nnodes 1 --nproc-per-node ${NPROC} vla-scripts/finetune.py \
    ...
    --wandb_entity "a10v-1" \
    --wandb_project "VLA-DFM" \
    --run_id_note "smoke-2xA100-3k--$(date +%Y%m%d_%H%M)" \
    | awk '{ print strftime("[%Y-%m-%d %H:%M:%S]"), $0; fflush(); }'
fi
